...
static ast_node *parse_switch_stmt(parser *self) {
    advance(self); // consume PULI
    expect(self, LEFT_PAREN, "Expected '(' after 'PULI'");
    ast_node *expr = parse_expression(self);
    expect(self, RIGHT_PAREN, "Expected ')' after switch expression");
    expect(self, NEWLINE, "Expected new line after switch declaration");

    int temp_size = 10;
    ast_node **cases = arena_alloc(self->arena, sizeof(ast_node *) * temp_size);
    int case_count = 0;
    while (match(self, KASO)) {
        ast_node *case_node = parse_case_stmt(self);
        cases[case_count++] = case_node;
        if (case_count >= temp_size) {
            temp_size *= 2;
            ast_node **new_cases = arena_alloc(self->arena, sizeof(ast_node *) * temp_size);
            memcpy(new_cases, cases, sizeof(ast_node *) * case_count);
            cases = new_cases;
        }
    }
    ast_node *switch_node = ast_new_node(self->arena, AST_SWITCH);
    switch_node->switch_stmt.expression = expr;
...
static ast_node *parse_case_stmt(parser *self) {
    advance(self); // consume KASO
    expect(self, LEFT_BRACE, "Expected '{' after 'KASO'");
    ast_node *value = parse_expression(self);
    expect(self, RIGHT_BRACE, "Expected '}' after case value");
    expect(self, COLON, "Expected ':' after case '}'");
    expect(self, NEWLINE, "Expected new line before case block");
    ast_node *block = parse_block(self);
...
ast_node *parser_parse(parser *self) {
    ...
    while (!is_at_end(self)) {
        if (match(self, KATAPUSAN)) {
            advance(self); // consume end keyword
            break;
        }
        if (match(self, NEWLINE)) {
            advance(self);
