...
void ast_print(ast_node *node, int depth) {
    ...
    case AST_SWITCH:
        ...
        print_indent(depth + 1);
        printf("Expression:\n");
        ast_print(node->switch_stmt.expression, depth + 2);
        for (int i = 0; i < node->switch_stmt.case_count; i++) {
            print_indent(depth + 1);
            printf("Case %d:\n", i + 1);
            ast_print(node->switch_stmt.cases[i], depth + 2);
        }
        break;
    ...
    case AST_CASE:
        ...
        print_indent(depth + 1);
        printf("Value:\n");
        ast_print(node->case_stmt.value, depth + 2);
        print_indent(depth + 1);
        printf("Block:\n");
        ast_print(node->case_stmt.block, depth + 2);
